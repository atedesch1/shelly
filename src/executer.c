...
void execute_command(program_call **program_calls, char **redirect_paths)
{
    ...
    set_erroutput(def_erroutput, &fd_erroutput, redirect_paths[2]);
    // redirect error output
    dup2(fd_erroutput, STDERR_FILENO);
    close(fd_erroutput);

    // sets input for first process
    set_input(def_input, &fd_input, redirect_paths[0]);
    ...
    for (program_call **p = program_calls; *p != NULL; p++)
    {
        ...
        { // last process
            ...
            set_output(def_output, &fd_output, redirect_paths[1]);
        }
        else
        {
            int fdpipe[2];
        ...
        if (child_pid == 0)
        { // child process
            if (execve((*p)->program, (*p)->params, (*p)->envparams) == -1)
            {
